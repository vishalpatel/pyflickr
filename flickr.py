...
THIS Project is originally adopted from 
    flickr.py
...
class Photo(object):
    ...
    __readonly = ['id', 'secret', 'server', 'isfavorite', 'license', 'rotation', 
                  ...
                  'cancomment', 'canaddmeta', 'comments', 'notes','tags', 'permcomment', 
                  'permaddmeta']
    ...
    def __init__(self, id, owner=None, dateuploaded=None, \
                 ...
                 canaddmeta=None, comments=None, notes=None,tags=None, secret=None, \
    ...
    def _load_properties(self):
        ...
        if hasattr(photo.notes, "note"):
            if isinstance(photo.notes.note, list):
                self.__notes = [Note(note.id, User(note.author), note.text,note.x,note.y,note.w,note.h) \
                               for note in photo.notes.note]
            else:
                note = photo.notes.note
                self.__notes = [Note(note.id, User(note.author), note.text,note.x,note.y,note.w,note.h)]
...
class Note(object):
    def __init__(self, id, author, text,x=0,y=0,w=0,h=0):
        self.id = id
        self.author = author
        self.text = text
        self.x=x
    ...
    def __str__(self):
        return '<Flickr Note %s (%s)>' % (self.id, self.text)
...
def _dopost(method, auth=False, **params):
    ...
    #print url
    #print payload
    rsp=urlopen(url, payload)
    print rsp.code
    return _get_data(minidom.parse(rsp))
